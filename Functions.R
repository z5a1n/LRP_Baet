...
BH_pars <- function(fit){
  mylist=list()
  mylist[[1]]=Rinf=mean(extract(fit)$Rinf)
  mylist[[2]]=Sq=mean(extract(fit)$Sq)
  mylist[[3]]=sig=mean(extract(fit)$sigma)
  names(mylist)=c("Rinf","Sq","sig")
  return(mylist)
}
rick_pars <- function(fit){
  mylist=list()
  mylist[[1]]=k=mean(extract(fit)$k)
  mylist[[2]]=Sq=mean(extract(fit)$Sk)
  mylist[[3]]=c=mean(extract(fit)$sigma)
  names(mylist)=c("k","Sk","sig")
  return(mylist)
}
sBH_pars <- function(fit){
  mylist=list()
  mylist[[1]]=Rinf=mean(extract(fit)$Rinf)
  mylist[[2]]=Sq=mean(extract(fit)$Sq)
  mylist[[3]]=c=mean(extract(fit)$c)
  mylist[[4]]=sig=mean(extract(fit)$sigma)
  names(mylist)=c("Rinf","Sq","c","sig")
  return(mylist)
}
SL_pars <- function(fit){
  mylist=list()
  mylist[[1]]=Rinf=mean(extract(fit)$k)
  mylist[[2]]=Sq=mean(extract(fit)$Sk)
  mylist[[3]]=c=mean(extract(fit)$c)
  mylist[[4]]=sig=mean(extract(fit)$sigma)
  names(mylist)=c("k","Sk","c","sig")
  return(mylist)
}
